...
export class AuthService {
  ...
  register(userInfo: RegisterUserInterface): Observable<boolean> {

    const url = `${this.myUrl}/auth/register`;

    return this.http.post<LoginResponse>(url, userInfo).pipe(

      tap((loginResp) => {
        const { email, password } = userInfo;
        this.login(email, password).subscribe();
        console.log('tapRegister');
